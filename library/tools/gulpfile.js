...
function jsTask() {
    return src([
            files.jsPath, '!' + 'src/js/jquery.min.js', // to exclude any specific files
        ])
...
// watch tasks 
function watchImg() {
    watch([files.imgPath], { interval: jsPollTime, usePolling: true }, //Makes docker work
        imgTask
    );
}

function watchTask() {
    watch([files.scssPath, files.jsPath], { interval: 2000, usePolling: true }, //Makes docker work
        series(
...
function watchAllScssTask() {
    watch([files.scssPath, files.jsPath, files.imgPath], { interval: 2000, usePolling: true }, //Makes docker work
        series(
...
function watchAllLessTask() {
    watch([files.lessWatchPath, files.jsPath, files.imgPath], { interval: 2000, usePolling: true }, //Makes docker work
        series(
...
function watchScss() {
    livereload.listen();
    watch([files.scssPath], { interval: cssPollTime, usePolling: true }, //Makes docker work
        scssTask
    );
}

function watchLess() {
    watch([files.lessWatchPath], { interval: cssPollTime, usePolling: true }, //Makes docker work
        lessTask
    );
}

function watchJs() {
    watch([files.jsPath], { interval: jsPollTime, usePolling: true }, //Makes docker work
        jsTask
    );
}
